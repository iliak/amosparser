...
def readExtension(stream):
    ...
    extension = known_extensions[extensions[slot]]
    if not extension:
        return "Extension {} unknown".format(extensions[slot])

    if token not in extension["tokens"]:
        return "[Extension: {} - Token: 0x{:04}] ".format(slot, token)

    return extension["tokens"][token]
